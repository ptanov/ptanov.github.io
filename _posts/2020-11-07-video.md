...
# based on https://stackoverflow.com/questions/18444194/cutting-the-videos-based-on-start-and-end-time-using-ffmpeg
	...
	CONCAT_LIST="$(printf "%s\n%s\ninpoint %s\noutpoint %s" "${CONCAT_LIST}" "file ${inputFile// /\\ }" "${startSeconds}" "${endSeconds}")"
}
...
- изцяло ново аудио: `ffmpeg -i v.mp4 -i a.wav -c:v copy -map 0:v:0 -map 1:a:0 -shortest new.mp4`, <https://superuser.com/questions/1137612/ffmpeg-replace-audio-in-video>
- микс <https://stackoverflow.com/questions/14498539/how-to-overlay-downmix-two-audio-files-using-ffmpeg>, <https://trac.ffmpeg.org/wiki/AudioChannelManipulation>
- безплатна музика за фон: [mixkit](https://mixkit.co/free-stock-music/), [besound](https://www.bensound.com/licensing)
- TODO: https://superuser.com/questions/1347946/ffmpeg-merge-audio-with-video-and-lower-volume https://superuser.com/questions/590201/add-audio-to-video-using-ffmpeg
